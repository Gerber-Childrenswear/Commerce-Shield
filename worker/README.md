...
Replace `<your-worker-origin>` with your deployed Worker URL (from `SHOPIFY_APP_URL` in `wrangler.toml`).
...
- Confirm Worker URL matches `SHOPIFY_APP_URL` in `wrangler.toml`.
